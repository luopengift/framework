...
// A RateLimit limits the rate at which an action can be performed.  It
...
func New(limit int, interval time.Duration) *RateLimit {
	lim := &RateLimit{
		limit:    limit,
		interval: interval,
	}
...
func (r *RateLimit) Try() (ok bool, remaining time.Duration) {
	r.mtx.Lock()
	defer r.mtx.Unlock()
	now := time.Now()
